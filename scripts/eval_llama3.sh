#!/bin/bash
# Rouge-L evaluation script - Test the performance of trained models on multiple datasets

# Set default parameters
MODEL_PATH="./checkpoints/llama-3.2-1b"
LORA_PATH="./outputs/expert_distill_llama3_8b_1b_20250726_165950_new_mc3/checkpoint-45740/adapter_model"
MODEL_TYPE="auto"
DATA_DIR="./data"
OUTPUT_DIR="./eval_results/eval_results_llama3_ours"
MAX_SAMPLES="None"
DEVICE="cuda"

# Set CUDA device (force the use of a single GPU to avoid device mismatch issues)
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --device)
            ...
            ;;
        --help|-h)
            echo "Usage: $0 --model_path <model path> [other options]"
            echo ""
            ...
            echo "  --output_dir       Results output directory, default: ./eval_results_<timestamp>"
            echo "  --max_samples      Maximum number of samples per dataset (for quick testing)"
...
if [ -z "$MODEL_PATH" ]; then
    echo "Error: --model_path parameter must be specified"
    echo "Use --help to see usage"
    exit 1
fi
...
if [ -n "$LORA_PATH" ] && [ ! -d "$LORA_PATH" ]; then
    echo "Warning: LoRA path does not exist: $LORA_PATH, LoRA weights will be ignored"
    LORA_PATH=""
fi
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "An error occurred during evaluation, please check the log information"
    exit 1
fi
