...
class Linear(nn.Linear, LoraLayer):
    ...
    def forward(self, x: torch.Tensor, task_types=None):
        if self.disable_adapters:
            ...
            # Apply MoE LoRA
            for i in range(self.lora_num):
                lora_output = getattr(self, f"lora_B{i}")(
                    getattr(self, f"lora_A")(self.lora_dropout(x))
                ) * self.scaling
                result = result + torch.unsqueeze(route_weight[:, :, i], -1) * lora_output
            
            # Optional: Calculate and store routing losses (for balancing experts)
            if self.training:
                # Calculate route balance loss
                routing_loss = self.cv_squared(route_weight.mean(dim=[0, 1]))
        else:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
